  ...
  return (
    <ul
      ...
      className={`flex flex-row w-full
				${standalone ? `${paddingStyles[size]} border-b-[0.5px] border-current/20 group-hover/calendar:border-current/30` : size === "small" ? "pt-1" : "pt-1.5"}
				${gapStyles[size]}
    ...
    >
      {weekdayLabels.map((day) => (
        <li key={day} className="flex-grow-1 w-full text-center">
          {day}
        </li>
      ))}
    </ul>
