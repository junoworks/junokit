...
export function MonthNavigation({ currentMonth, onMonthChange, size, standalone, ...rest }: MonthNavigationProps) {
  ...
  return (
    <div {...rest} className={`flex flex-row justify-between items-center w-full ${standalone ? paddingStyles[size] : "p-0"} ${rest.className}`}>
      <Button icon={<ChevronLeft />} variant="ghost" size="mini" aria-label="Previous month" onClick={() => onMonthChange(subMonths(currentMonth, 1))} />
      <MonthYearSelector currentMonth={currentMonth} size={size} onMonthChange={onMonthChange} />
      <Button icon={<ChevronRight />} size="mini" variant="ghost" aria-label="Next month" onClick={() => onMonthChange(addMonths(currentMonth, 1))} />
    </div>
  );
}
