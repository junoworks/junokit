	...
	while (weeks.length < 6) {
				...
				weeks.push(
					<div role="row" className={`flex w-full gap-0 ${radiusClass} justify-evenly`} key={weeks.length}>
						{Array.from({ length: 7 }).map((_, i) => (
							<div
								key={i}
								className={`${cellHeight} flex items-center justify-center aspect-square ${!isCircular ? "w-full flex-grow" : ""}`}
							/>
						))}
					</div>,
				);
		...
		for (let i = 0; i < 7; i++) {
			...
			const inSelectedRange =
				...
				(isAfter(day, startDate) || isEqual(day, startDate)) &&
				(isBefore(day, endDate) || isEqual(day, endDate));
			...
			const inHoverRange =
				...
				(isAfter(day, startDate) || isEqual(day, startDate)) &&
				(isBefore(day, hoveredDate) || isEqual(day, hoveredDate));
