...
import { type ChangeEvent, forwardRef, type KeyboardEvent, useEffect, useId, useImperativeHandle, useRef, useState } from "react";
import { Button } from "../..";
...
export type MiniCalendarProps = MiniCalendarSpecificProps & Omit<React.HTMLAttributes<HTMLDivElement>, keyof MiniCalendarSpecificProps>;
...
const MiniCalendar = forwardRef<HTMLInputElement, MiniCalendarProps>((props, ref) => {
  const { standalone, color, radius, weekStart, size, className, style, value, onChange, inputProps, id, ...containerProps } = { ...defaults, ...props };
  ...
  return (
    <div {...containerProps} className={classes} id={id} style={style} role="application" aria-label="Calendar" onKeyDown={handleKeyDown} onBlur={handleBlur}>
      <MonthNavigation currentMonth={currentMonth} onMonthChange={setCurrentMonth} size={size} standalone={standalone} />
      ...
      >
        <DaysGrid
          ...
          minDate={inputProps?.min ? (parseDateFromInput(String(inputProps.min)) ?? undefined) : undefined}
          maxDate={inputProps?.max ? (parseDateFromInput(String(inputProps.max)) ?? undefined) : undefined}
        />
      </div>

      {/* Footer */}
      <div className={`w-full ${!standalone ? "p-0 pt-1" : paddingStyles[size].innerBottom} flex justify-between items-center ${gapStyles[size].bottom}`}>
