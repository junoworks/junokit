...
export default function Combobox(props: ComboboxProps) {
	...
	const clearButton = showClearButton && (
		<Button
			...
			className={`${buttonOffset} ${showClear ? "" : "invisible"}`}
		/>
	);

	const dropdownClasses = [
		"absolute left-0 right-0 z-50 flex flex-col overflow-y-auto",
		"bg-base-0 border border-base-200 shadow-lg",
				...
				{open && (
					<div ref={contentRef} className={dropdownClasses} style={{ maxHeight, scrollbarWidth: "thin" }}>
						...
						{filteredOptions.length === 0 && (
							<div className={`${itemBaseClasses} text-current/50 cursor-default`}>no results</div>
						)}
					</div>
				)}
			</div>
